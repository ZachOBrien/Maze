...
(module+ main
  ...
  (define dir (convert-direction dir-ip))
  (define degree degree-ip)
  (define state-after-shift (gamestate-shift-and-insert state index dir degree))

  (write-json
   (map gridposn->hash (sort (all-reachable-from state) compare-row-col))))