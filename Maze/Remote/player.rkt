...
(define/contract proxy-player% player-interface
  (class object%
    ...
    (define/public (propose-board min-num-rows min-num-cols)
      ...
      (hash->board (send tcp-conn receive-json)))
  
    ;; PlayerState GridPosn -> Any
    ;; Sets initial state and treasure position
    (define/public (setup plyr-state new-goal)
      (send tcp-conn send-json (list "setup" (list (public-player-state->hash plyr-state)
                                                   (gridposn->hash new-goal))))
    ...
    (define/public (take-turn plyr-state)
      (send tcp-conn send-json (list "take-turn" (list (public-player-state->hash plyr-state))))
      (hash->action (send tcp-conn receive-json)))
